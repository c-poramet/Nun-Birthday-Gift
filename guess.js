document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', function(e) {
        ...
        // Color and number
        guesses.favoriteColor = colorInput.value;
        guesses.favoriteNumber = parseFloat(numberSlider.value).toFixed(3);

        // DND class
        guesses.favoriteDNDClass = formData.get('favoriteDNDClass');

        // 5 coordinate system questions
        guesses.planFlexX = parseFloat(formData.get('planFlexX'));
        guesses.introExtroY = parseFloat(formData.get('introExtroY'));
        guesses.orderChaosX = parseFloat(formData.get('orderChaosX'));
        guesses.logicEmotionY = parseFloat(formData.get('logicEmotionY'));
        guesses.leaderSupportX = parseFloat(formData.get('leaderSupportX'));
        guesses.optimistRealistY = parseFloat(formData.get('optimistRealistY'));
        guesses.fastThoroughX = parseFloat(formData.get('fastThoroughX'));
        guesses.riskCautiousY = parseFloat(formData.get('riskCautiousY'));
        guesses.techNatureX = parseFloat(formData.get('techNatureX'));
        guesses.minCollectorY = parseFloat(formData.get('minCollectorY'));

        // The rest as dropdowns
        guesses.favoriteProvince = formData.get('favoriteProvince');
        guesses.favoriteMovie = formData.get('favoriteMovie');
        guesses.favoriteSongGenre = formData.get('favoriteSongGenre');
        guesses.favoriteFood = formData.get('favoriteFood');
        guesses.favoriteAnimal = formData.get('favoriteAnimal');
        guesses.favoriteEmoji = formData.get('favoriteEmoji');
        guesses.favoriteSeason = formData.get('favoriteSeason');
        ...
        alert(`Guesses saved! Thanks ${guesserName}! File downloaded successfully.`);
    });
